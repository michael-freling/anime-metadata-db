...
> **Amended 2026-08-23.** The shape held, the rule did not: an appearance's `voiceActors[]` **adds
> to** the character's rather than replacing them. Replacing meant one English dub forced every
> appearance to restate the Japanese cast, and forgetting to dropped it silently — which happened to
> Saber. See [Using the API](/docs/using-the-api) for the behaviour as shipped.
