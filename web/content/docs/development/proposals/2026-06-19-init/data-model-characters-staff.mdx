...
**Status:** Design input — companion to [Anime Series/Franchise Metadata Research](/docs/development/proposals/2026-06-19-init/anime-metadata-research)
**Related:** [Anime Series Data Model](/docs/development/proposals/2026-06-19-init/data-model-anime-series) — the R1 spine this joins onto.
...
**many-to-many** node onto the [Anime Series Data Model](/docs/development/proposals/2026-06-19-init/data-model-anime-series). They are also
...
`appearances`, which name a `seriesId` from the [Anime Series Data Model](/docs/development/proposals/2026-06-19-init/data-model-anime-series).
...
### 2.3 Voice actors: constant on the Character, added to per appearance
...
> **Amended 2026-08-23.** The shape held; three things about the rule did not.
>
> 1. **An appearance's `voiceActors[]` adds to the character's rather than replacing them.**
>    Replacing meant one English dub forced every appearance to restate the Japanese cast, and
>    forgetting to dropped it silently — which is what happened to Saber. So the character carries
>    the cast that holds *throughout* (an original-language role is nearly always the same person in
>    every adaptation) and an appearance carries only what is specific to it.
> 2. **What a `Character` reports depends on what was asked.** `ListCharacters` with a `seriesId`,
>    and the cast embedded in `GetSeries`, resolve `voiceActors[]` to *that series'* cast — the
>    constant cast plus anyone cast only there. Unscoped there is no series to be specific to, so
>    only the constant cast is reported and the rest stays on the appearances. An appearance's own
>    `voiceActors[]` is always the resolved list, so no client ever merges the two itself.
> 3. **A character may hold more than one appearance in the same series**, distinguished by
>    `scope[]` (§2.2), when the cast changes between its installments: Saber is in every *Fate/stay
>    night* adaptation, but Kate Higgins dubbed only the 2006 one. An appearance is therefore
>    identified by `seriesId` **and** `scope[]`, not by `seriesId` alone. The credits index is
>    coarser on purpose — it keys by series — so a staff credit reads "Saber in Fate/stay night"
>    even when the appearance behind it is narrower.
>
> See [Using the API](/docs/using-the-api) for the behaviour as shipped.
...
[Anime Series Data Model §3.2](/docs/development/proposals/2026-06-19-init/data-model-anime-series#32-fate--numbered-vs-date-ordered-series-in-one-franchise)).
...
1. **Seed** characters and their voice actors from each media node's AniList character/VA edges —
   the media IDs are already in the [Anime Series Data Model](/docs/development/proposals/2026-06-19-init/data-model-anime-series)
   (`externalIds.anilistId`).
