...
| `series.titles` | mixed, and the one row in this table whose licence differs *by key*. `original` and the romanization are this project's — a series spans several upstream entries and matches none of them, so nothing upstream can supply them. `translations.en` is Wikidata's: the `P1476` title claim of the work named by `series.externalIds.wikidataId`, falling back to that entity's `en` label, filled only where the series names a work and refused where the candidate turns out to be the romanization respelled | ODbL, except `en` which is **CC0** |
...
| `*.externalIds.wikidataId` | this project — the authored QID join key, carried by cast nodes and by the series that name a Wikidata work | ODbL |
...
- `series.externalIds.anidbId`, `series.externalIds.tvdbId`,
  ...
  catalogues. Only `wikidataId` is read from a series, because Wikidata is the
  one source that models the *work* rather than each broadcast run.
