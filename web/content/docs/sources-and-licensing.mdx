...
| `*.externalIds.anilistId` | anime-offline-database — resolved at build time by finding the entries that carry the series' own title among their own, and dealing them out to its installments in airing order; the id itself is parsed from that entry's `sources` array. An authored id overrides the resolution, and is this project's | ODbL + DbCL |
