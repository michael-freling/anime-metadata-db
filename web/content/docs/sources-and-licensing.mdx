...
| `*.externalIds.anilistId` | anime-offline-database — resolved, not authored: the build finds the entries carrying the series' own native title and orders them by airing date, filling each installment's join key. 492 of 699 come this way; the 207 upstream does not list under the series title stay authored, and the resolution still checks any id an override names. Absent, not zero, where upstream lists the work on no AniList page at all — the entry it resolved to still fills every other fact | ODbL + DbCL |
