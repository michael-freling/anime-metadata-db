...
| `*.externalIds.anilistId` | this project — the authored join key on whichever node carries it. The build independently resolves what it should be, by finding the entries carrying the series' own title, and reports a disagreement; where nothing is authored it fills from that resolution instead | ODbL |
