...
| `ListFranchises` | `{"limit": 10}` | multi-series franchises, fully nested, one page at a time |
...
reachable through `Search` and `GetSeries`. Because a franchise carries its whole
tree — every series, season and episode — it is paginated like the other lists,
and `ListCatalog` is the cheaper way to ask what the dataset contains. For
`Search`, `limit` is optional (defaults to 50) and a blank `query` matches
nothing. For the list methods `limit` defaults to 100; an unknown `seriesId` is a
