...
const coverageDoc = "src/web/content/docs/coverage.mdx"

func TestCoveragePageMatchesTheDataset(t *testing.T) {
	// The repository root, three levels up from this package. dataset/ holds
	// the records the index is built from; src/web/ holds the page being
	// checked, so both are reached from here.
	const root = "../../../.."
	d, err := Load(os.DirFS(root + "/dataset"))
	if err != nil {
