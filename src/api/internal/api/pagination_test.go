...
import (
	...
	browsev1 "github.com/michael-freling/anime-metadata-db/src/api/internal/gen/browse/v1"
...
// committedDataset is the repository's dataset/ directory, reached from this
// package. The dataset is read from disk rather than embedded, so a test that
// wants the real catalogue has to say where it is.
const committedDataset = "../../../../dataset"

func realStore(t *testing.T) *Store {
	t.Helper()
	fsys, idx, err := animedb.Open(committedDataset)
	if err != nil {
