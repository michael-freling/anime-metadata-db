...
module github.com/michael-freling/anime-metadata-db/src/builder

go 1.25.0

require (
	github.com/michael-freling/anime-metadata-db/src/animedb v0.0.0
...
// The shared module lives beside this one in the same repository; it is never
// consumed from a proxy, so it is always resolved from the working tree.
replace github.com/michael-freling/anime-metadata-db/src/animedb => ../animedb
